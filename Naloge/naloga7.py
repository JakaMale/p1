...
def osebe(obiski):
    return set([imena for imena, aktivnost in obiski])
def aktivnosti(obiski):
    return set([aktivnost for imena, aktivnost in obiski])
def udelezenci(aktivnost, obiski):
    return set([ime for ime, akti in obiski if akti == aktivnost])
def po_aktivnostih(obiski):
    return  {aktivnost: udelezenci(aktivnost, obiski) for imena, aktivnost in obiski }
def skupine(obiski):
    return [po_aktivnostih(obiski)[i] for i in po_aktivnostih(obiski)]
def okuzeni(skupine, nosilci):
   mby_kuzni = set()
   for i in skupine:
       if i-nosilci != i:
         mby_kuzni.update(i- nosilci)
   return mby_kuzni
    ...
    for i in skupine:
        for b in i:
            if b not in a:
                a[b]=0
            a[b] += len(i)-1
    return next(iter(sorted(a.items(), key=lambda x: (-x[1], x[0]))))[0]
