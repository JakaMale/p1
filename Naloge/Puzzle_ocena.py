...
def obrati(kos):
    res = set()
    for j in range(4):
        res.add(obrnjen_n(kos, j))
        res.add(obrnjen_n(zrcaljen_vod(kos),j))
    return res
...
def kotni(kosi):
    res=set()
    zbir = zbirka_stranic(kosi)
    for i in kosi:
        s=stranice(i)
        if len(zbir[s[0]])==1 and len(zbir[s[3]])==1:
            res.add(i)
        if len(zbir[s[0]])==1 and len(zbir[s[1]])==1:
            res.add(i)
        if len(zbir[s[2]]) == 1 and len(zbir[s[3]]) == 1:
            res.add(i)
        if len(zbir[s[2]]) == 1 and len(zbir[s[1]]) == 1:
            res.add(i)
    return res
...
def robni(kosi):
    ...
    for i in kosi:
        s=stranice(i)
        if len(zbir[s[0]])==1 and len(zbir[s[3]])!=1 and len(zbir[s[1]])!=1:
            res.add(i)
        if len(zbir[s[1]])==1 and len(zbir[s[0]])!=1 and len(zbir[s[2]])!=1:
            res.add(i)
        if len(zbir[s[2]]) == 1 and len(zbir[s[3]])!=1 and len(zbir[s[1]])!=1:
            res.add(i)
        if len(zbir[s[3]]) == 1 and  len(zbir[s[0]]) != 1 and  len(zbir[s[2]]) != 1:
            res.add(i)
    return res
...
def preobrni(kos, levo, gor, po_stranicah):
    ...
    moznosti=obrati(kos)
    for i in moznosti:
        moznosti.add(zrcaljen_navp(i))
    moznosti=list(moznosti)
    for i in moznosti:
        stra=stranice(i)
        if levo == 1:
            if len(po_stranicah[stra[3]])==1  and gor == stra[0]:
                return i
        elif gor == 1:
            if len(po_stranicah[stra[0]]) == 1  and levo == stra[3]:
                return i
        else:
            if  levo == stra[3] and gor == stra[0]:
                return i

def vzemi_kos(levo, gor, po_stranicah, uporabljeni):

    for i in po_stranicah:
        for j in po_stranicah[i]:
            moznosti = obrati(j)
            for i in moznosti:
                moznosti.add(zrcaljen_navp(i))
            moznosti = list(moznosti)
            for a in moznosti:
                stra = stranice(a)
                if levo == 1:
                    if len(po_stranicah[stra[3]]) == 1 and gor == stra[0] and j not in uporabljeni:
                        uporabljeni.add(j)
                        return a
                elif gor == 1:
                    if len(po_stranicah[stra[0]]) == 1 and levo == stra[3] and j not in uporabljeni:
                        uporabljeni.add(j)
                        return a
                else:
                    if levo == stra[3] and gor == stra[0] and j not in uporabljeni:
                        uporabljeni.add(j)
                        return a
...
def sestavi_koscke(kosi, kos):
    ...
    while kosi:
        i = 0
        primerjaj=copy.deepcopy(temp)
        if not temp:
            while i<len(kosi):
                vsi = obrati(kosi[i])
                if not temp:
                    for element in vsi:
                        if stranice(element)[0] == stranice(rez[index - 1][0])[2]:
                            temp.append(element)
                            kosi.remove(kosi[i])
                i += 1
        if temp and kosi:
            while i < len(kosi):
                vsi = obrati(kosi[i])
                for element in vsi:
                    if stranice(element)[3]==stranice(temp[-1])[1]:
                        temp.append(element)
                        kosi.remove(kosi[i])
                        i-=1
                        break
...
def slika(kosi):
    ...
    for sez in kosi:
        ...
        for ele in sez:
            piki=[]
            for rabm in ele[1:-1]:
                piki.append(rabm[1:-1])
            seznamck.append(piki)
        obrezani.append(seznamck)


    for sezi in obrezani:
        stringci=[""]*len(obrezani[0][0])
        for elemen in sezi:
            for index,delck in enumerate(elemen):
                stringci[index] +=delck
        for sest in stringci:
            skupni +=sest+"\n"

    return skupni

def sestavljanka(kosi, kot):
    kosi1=sestavi_koscke(kosi,kot)
    return (slika(kosi1))

######################################################################################################
############ Lepa naloga sm men se res ni dal optimizerat. Kr se morm uČit matematiko#################
############ Lepe praznike :D tistmu ku bo tu gledou #################################################
    ...
    def test_zbirka_stranic(self):
        self.assertEqual({
            ...
            '55mQ': {('Qm55', 'Ro/k', 'b \\C', 'Km77'),
                     ('Qm55', '7 _H', '2o\\R', 'H3cc')},
            'cRH5': {('Qm55', '7 _H', '2o\\R', 'H3cc')},        #wJMY', 'w  Y', '2  T', 'Fnyq,  '79jc', '7  c', 'm\\ x', 'K9ZK',  Fnyq', 'F  q', '8 (z', '4MJn ,  K9ZK', 'K/ K', 'O) a', '40YO'
