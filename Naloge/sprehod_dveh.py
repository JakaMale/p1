...
def v_pot(s):
    ...
    for i in s:
        znaki.append(i[0])
        stevilke.append(i[1:])
    stevilke = list(map(int, stevilke))
    a = list(zip(znaki,stevilke))
    return a

def odsek(x, y, smer, razdalja):
    sez= []
    for i in range(razdalja+1):
        sez.append((x, y))
        if smer == '<':
            x-=1
        elif smer == '>':
...
def tocke(s):
    ...
    y = 0
    a=[0]
    for smer,stevilo in s:
        a.pop(-1)
        a=a+(odsek(x, y, smer, stevilo))
        x = odsek(x, y, smer, stevilo)[-1][-2]
        y = odsek(x, y, smer, stevilo)[-1][-1]
    return a

def presecisca(s, t):
    se_list_s = set(tocke(v_pot(s)))
    se_list_t=set(tocke(v_pot(t)))
    tuple_rez=se_list_s&se_list_t
    sez=list(tuple_rez)
    return sez

def tocke2(s):
    tocke_s_koraki = tocke(s)
    tockae=0,
    novo=[xs + (i,) for i,xs in enumerate(tocke_s_koraki)]
    novo.pop(0)
    return set(novo)


def prvo_srecanje(t,s):
    t=v_pot(t)
    s=v_pot(s)
    a=list(tocke2(t))
    b=tocke(s)
    b=set(b)
    new=[item for item in a if item[:2] in b]
    a2=set(tocke(t))
    b2=list(tocke2(s))
    new2 = [item for item in b2 if item[:2] in a2]
    koncni_list=[]
    for i in new:
        skupaj=0
        for j in new2:
            if j[:2]==i[:2]:
                if j[-1]>=i[-1]:
                    ...
                    skupaj = i[-1]
                koncni_list.append((skupaj,(j[:2])))

    return min(koncni_list, key=lambda t:t[0])
