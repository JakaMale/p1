...
def vsa_drevesa(gozd):
    return set((j,i)for i, vrsta in enumerate(gozd) for j, znak in enumerate(vrsta) if je_drevo(j, i, gozd))

def stevilo_dreves(x0, y0, x1, y1, gozd):
    return  sum(1 for i in range(x0,x1) for j in range(y0,y1) if je_drevo(i,j,gozd))

def ni_dreves(x0, y0, x1, y1, gozd):
    return 0==stevilo_dreves(x0, y0, x1, y1, gozd)

def dreves_na_33(x, y, gozd):
    return stevilo_dreves(x,y,x+3,y+3,gozd)

def naj_kvadrat(gozd):
    return max(dreves_na_33(i,j,gozd) for i in range(dimenzije(gozd)[0]-2) for j in range(dimenzije(gozd)[1]-2))
...
def lovec(x, y, pot, gozd):
    ...
    for korak in pot:
        if korak == '>':
            if x+1 <= dim[0]-1:
                if je_drevo(x+1,y,gozd):
                    buske+=1
                elif x<dim[0]:
                     x+=1
        elif korak == '<':
            if x - 1 >= 0:
                if not je_drevo(x - 1, y, gozd):
                    x -= 1
                elif x>0:
                    buske +=1
        if korak == '^':
            if y - 1 >= 0:
                if je_drevo(x , y-1, gozd):
                    buske += 1
                elif y > 0:
                    y -= 1
        elif korak == 'v':
...
def zajec(drevesa):
    return max((Counter(x for x,y in drevesa)), key=(Counter(x for x,y in drevesa)).get),max((y for x,y in drevesa if x==max((Counter(x for x,y in drevesa)), key=(Counter(x for x,y in drevesa)).get)))

def razdalja(x0, y0, x1, y1):
    return sqrt((x0-x1)**2+(y0-y1)**2)

def bliznja_drevesa(x, y, r, drevesa):
    return set((x1,y1) for x1,y1 in drevesa  if razdalja(x, y, x1, y1)<=r and razdalja(x, y, x1, y1)!=0)

def povezana_drevesa(r, drevesa):
    return (dict(((x,y),bliznja_drevesa(x,y,r,drevesa)) for x,y in drevesa))

def jasa(gozd):
    return max([i*j for i in range(1,dimenzije(gozd)[0]+1) for j in range(1,dimenzije(gozd)[1]+1) for x in range (dimenzije(gozd)[0]+1-i) for y in range(dimenzije(gozd)[1]+1-j) if stevilo_dreves(x,y, x+i,y+j,gozd) == 0])
...
def opica(x, y, r, drevesa):
    ...
    while True:
        x,y = seez[j]
        a = list(bliznja_drevesa(x,y,r,drevesa))
        for i in a:
            if i not in seez:
                seez.append(i)
        if j < len(seez)-1:
